...
{-@ mkNode :: color:Color
             ...
             -> {v:RBT a | isRBT v &&
                           ...
                           color == getColor v}
@-}
mkNode :: Color -> RBT a -> a -> RBT a -> RBT a
mkNode color left val right = Node color left val right
...
{-@ makeBlack :: t:{RBT a | isWeak t}
              -> v:{RBT a | isRBT v}
@-}
makeBlack :: RBT a -> RBT a
makeBlack (Node _ left val right) = mkNode Black left val right
makeBlack Leaf = Leaf
...
{-@ balanceInsertL :: (Ord a) =>
                   ...
                   -> v:{RBT a | isWeak v &&
                                 ...
                                 getBlackHeight v == (if color == Black then 1 else 0) + getBlackHeight left}
@-}
balanceInsertL :: (Ord a) => Color -> RBT a -> a -> RBT a -> RBT a
balanceInsertL Black (Node Red (Node Red a x b) y c) z d = Node Red (Node Black a x b) y (Node Black c z d)
balanceInsertL Black (Node Red a x (Node Red b y c)) z d = Node Red (Node Black a x b) y (Node Black c z d)
...
balanceInsertR Black a x (Node Red (Node Red b y c) z d) = Node Red (Node Black a x b) y (Node Black c z d)
balanceInsertR Black a x (Node Red b y (Node Red c z d)) = Node Red (Node Black a x b) y (Node Black c z d)
